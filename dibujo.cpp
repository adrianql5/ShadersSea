...
void lanzarProyectil(unsigned int VAO, glm::vec3 origen, glm::vec3 vectorDirectorCamara, std::vector<Objeto> &barcosActivos, std::vector<Objeto> &proyectilesActivos, float velocidad) {
...
void generarBarcoActivo(unsigned int VAO, glm::vec3 origen, glm::vec3 vectorDirectorBarco, std::vector<Objeto> &barcosActivos, float velocidad) {
	...
	barcoNuevo.hitbox = { 80, 40, 40 }; // Tamaño del hitbox (ajustar según sea necesario)
